...
const StartupCard = ({ startup, variant = "bounty" }: StartupCardProps) => {
  ...
  const handleInvest = async () => {
    try {
      // Convert amount from USD to ETH (simplified for demo)
  ...
  if (variant === "bounty") {
    return (
      <>
        <Card 
          className="w-full overflow-hidden bg-[#0B0B13] border-[#1E1E2A] hover:border-[#2A2A3A] transition-all duration-200 cursor-pointer"
          onClick={handleClick}
        >
          {startup.banner_url && (
          ...
          <CardContent className="pt-3">
            ...
            <button 
              className="w-full bg-gradient-to-r from-[#267871] to-[#136a8a] hover:from-[#1b5d5a] hover:to-[#0f566e] text-white px-4 py-2 rounded-md font-medium transition-colors"
