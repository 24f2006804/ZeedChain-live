...
import { KycStatus } from '@/client/components/kyc/KycStatus';
import { BasicKycForm } from '@/client/components/kyc/BasicKycForm';
import { AdvancedKycForm } from '@/client/components/kyc/AdvancedKycForm';
...
export default function KycPage() {
  ...
  const { kycData, KycLevel, KycStatus: KycStatusEnum } = useKyc();
  const [activeTab, setActiveTab] = useState<KycTab>(KycTab.STATUS);
  
  // Switch to status tab when KYC is pending or approved
  useEffect(() => {
    if (kycData && (kycData.status === KycStatusEnum.PENDING || kycData.status === KycStatusEnum.APPROVED)) {
      setActiveTab(KycTab.STATUS);
    }
  }, [kycData, KycStatusEnum.PENDING, KycStatusEnum.APPROVED]);
